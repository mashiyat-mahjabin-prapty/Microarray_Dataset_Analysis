...
os.chdir('F:\Thesis\MicroArray dataset\Multiclass\Glioma\RFECV')
...
selected_features = []
# read data
df = pd.read_csv('F:\Thesis\MicroArray dataset\Multiclass\Glioma\Glioma.csv')
...
# get the feature importance using XGBoost and take top 10% of the features
model = XGBClassifier()
model.fit(X, y)
...
importance = importance.head(int(X.shape[1]*0.01))
...
for i in random_states:
    ...
    scoring = ['accuracy', 'balanced_accuracy', 'precision_weighted', 'recall_weighted', 'f1_weighted', 'roc_auc_ovr', 'matthews_corrcoef']
    for clf, label in zip([clf1, clf2, clf3, voting, stacking], ['Logistic Regression', 'SVM', 'Random Forest', 'Voting', 'Stacking']):
        ...
        results.write('\n')
        results.write('Precision: %0.2f (+/- %0.2f)' % (scores['test_precision_weighted'].mean(), scores['test_precision_weighted'].std() * 2))
        results.write('\n')
        results.write('Recall: %0.2f (+/- %0.2f)' % (scores['test_recall_weighted'].mean(), scores['test_recall_weighted'].std() * 2))
        results.write('\n')
        results.write('F1: %0.2f (+/- %0.2f)' % (scores['test_f1_weighted'].mean(), scores['test_f1_weighted'].std() * 2))
        results.write('\n')
        results.write('ROC AUC: %0.2f (+/- %0.2f)' % (scores['test_roc_auc_ovr'].mean(), scores['test_roc_auc_ovr'].std() * 2))
        results.write('\n')
        ...
        if label == 'Logistic Regression':
            ...
            precisions_lr.append(scores['test_precision_weighted'])
            recalls_lr.append(scores['test_recall_weighted'])
            f1_scores_lr.append(scores['test_f1_weighted'])
            roc_aucs_lr.append(scores['test_roc_auc_ovr'])
            mccs_lr.append(scores['test_matthews_corrcoef'])
            # if the current f1_weighted is greater than the previous f1_weighted, save the model
            if scores['test_f1_weighted'].mean() > best_f1_score:
            ...
            precisions_svm.append(scores['test_precision_weighted'])
            recalls_svm.append(scores['test_recall_weighted'])
            f1_scores_svm.append(scores['test_f1_weighted'])
            roc_aucs_svm.append(scores['test_roc_auc_ovr'])
            ...
            precisions_rf.append(scores['test_precision_weighted'])
            recalls_rf.append(scores['test_recall_weighted'])
            f1_scores_rf.append(scores['test_f1_weighted'])
            roc_aucs_rf.append(scores['test_roc_auc_ovr'])
            ...
            precisions_voting.append(scores['test_precision_weighted'])
            recalls_voting.append(scores['test_recall_weighted'])
            f1_scores_voting.append(scores['test_f1_weighted'])
            roc_aucs_voting.append(scores['test_roc_auc_ovr'])
            ...
            precisions_stacking.append(scores['test_precision_weighted'])
            recalls_stacking.append(scores['test_recall_weighted'])
            f1_scores_stacking.append(scores['test_f1_weighted'])
            roc_aucs_stacking.append(scores['test_roc_auc_ovr'])
