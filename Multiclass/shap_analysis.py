...
def shap_analysis(model, X, y, feature_names, dataset, class_names):
    ...
    plt.figure()
    # generate shap summary plot as beeswarm
    shap.plots.beeswarm(explainer(X), max_display=10, show=False)
    # save the shap summary plot
    plt.tight_layout()
    plt.savefig(dataset + '\shap\shap_summary_plot_beeswarm.jpg')
    plt.close()

    plt.figure()
    # generate shap summary plot as bar
    shap.plots.bar(explainer(X), max_display=10, show=False)
    # save the shap summary plot
    plt.tight_layout()
    plt.savefig(dataset + '\shap\shap_summary_plot_bar.jpg')
    plt.close()
    return
